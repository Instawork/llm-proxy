...
func (f *Server) handle(w http.ResponseWriter, r *http.Request) {
	...
	case "DeleteItem":
		// Actually drop the item so round-trip assertions
		// (CreateKey → DeleteKey → GetKey-not-found) work in tests.
		// Previously this was a no-op stub that always returned 200
		// regardless of whether the item ever existed.
		tableName, _ := input["TableName"].(string)
		key, _ := input["Key"].(map[string]any)
		pk := ExtractDDBString(key, "pk")
		if f.tables[tableName] != nil {
