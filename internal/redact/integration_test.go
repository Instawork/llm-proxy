...
//     stock recognizers and the custom Instawork ones disappear.
...
func TestIntegration_RecognizersYAMLIsMounted(t *testing.T) {
	...
	mustHave := []string{
		"InstaworkDobRecognizer",
		"InstaworkCheckrCandidateIdRecognizer",
		"InstaworkFullAddressRecognizer",
	}
...
// TestIntegration_InstaworkDOBRedacts exercises the InstaworkDobRecognizer
...
func TestIntegration_InstaworkDOBRedacts(t *testing.T) {
	r := newIntegrationRedactor(t, DefaultEntityTypes)
	...
	if !strings.Contains(res.Text, "[REDACTED:INSTAWORK_DOB]") {
		t.Errorf("expected INSTAWORK_DOB marker; got %q (recognizers.yaml not "+
...
// TestIntegration_InstaworkCheckrIDRedacts asserts that a UUIDv4 in
// "checkr candidate" context redacts as INSTAWORK_CHECKR_CANDIDATE_ID
// rather than as US_DRIVER_LICENSE (which the stock recognizer would
// otherwise hit at score 0.30 on UUID fragments).
func TestIntegration_InstaworkCheckrIDRedacts(t *testing.T) {
	r := newIntegrationRedactor(t, DefaultEntityTypes)

	const candidateUUID = "a1b2c3d4-e5f6-4789-a012-b3c4d5e6f789"
	res, err := r.Redact(context.Background(),
		"checkr candidate "+candidateUUID)
	if err != nil {
		t.Fatalf("Redact: %v", err)
	}
	if !strings.Contains(res.Text, "[REDACTED:INSTAWORK_CHECKR_CANDIDATE_ID]") {
		t.Errorf("expected INSTAWORK_CHECKR_CANDIDATE_ID marker; got %q "+
			"counts=%v", res.Text, res.EntityCounts)
	}
	if strings.Contains(res.Text, candidateUUID) {
		t.Errorf("raw UUID leaked: %q", res.Text)
	}
}

// TestIntegration_InstaworkFullAddressRedacts asserts that the strict
...
func TestIntegration_InstaworkFullAddressRedacts(t *testing.T) {
	r := newIntegrationRedactor(t, DefaultEntityTypes)
	...
	if !strings.Contains(res.Text, "[REDACTED:INSTAWORK_FULL_ADDRESS]") {
		t.Logf("INSTAWORK_FULL_ADDRESS not detected; spaCy LOCATION may have "+
