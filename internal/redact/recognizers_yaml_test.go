...
// custom recognizers specifically: a “type: custom“ entry
...
func TestRecognizersYAML_CustomRecognizersHavePatterns(t *testing.T) {
	...
	for _, r := range f.Recognizers {
		...
		if !strings.HasSuffix(r.Name, "Recognizer") {
			t.Errorf("custom recognizer %q does not use the 'Recognizer' name "+
				"suffix; please name custom recognizers consistently to keep "+
