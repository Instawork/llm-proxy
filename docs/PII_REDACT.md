...
The proxy can call a Microsoft Presidio analyzer sidecar to redact request
bodies before they are written to logs, cost-tracking transports, or any
other persistence layer. **The unredacted body still reaches the upstream
LLM** — model quality and tool-call accuracy are preserved. Only the copy
we keep for observability ever sees the redactor.
...
| `timeout_ms`      | `int`     | `200`         | Per-/analyze deadline. Tune downward after baselining sidecar p95.                            |
...
client ─▶ llm-proxy ──▶ MetaURLRewriting
                  ...
                  ├──▶ TokenParsing  ──▶ cost transports (read redacted body
                  │                         via PIIRedactedBody(ctx))
                  ├──▶ Streaming
                  └──▶ provider proxy ─▶ upstream LLM
```

Downstream consumers (cost transports, structured loggers) opt in by
calling `middleware.PIIRedactedBody(r.Context())`. Until they do, they
get nothing — there is no implicit "redacted body" overwrite of `r.Body`,
since that would change what reaches the upstream LLM.
...
and others — because they fire false positives on routine Instawork
data (10-digit worker IDs, ISO timestamps, etc.).
...
that's the per-request analyzer round trip. Once p99 is comfortably
under `timeout_ms`, you can tune the timeout downward.
