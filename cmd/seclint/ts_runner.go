...
func lintTypeScriptRepo(webDir string) ([]Finding, error) {
	script := filepath.Join(webDir, "scripts", "seclint.ts")
	...
	buildDir, err := filepath.Abs(filepath.Join(webDir, "scripts", ".seclint-build"))
	if err != nil {
		return nil, err
	}
	...
	outJS := filepath.Join(buildDir, "seclint.js")
	cmd := exec.Command(
		tsc,
		"--target", "ES2022",
		"--module", "NodeNext",
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		return nil, fmt.Errorf("tsc seclint: %v\n%s", err, out)
	}
	...
	if err := node.Run(); err != nil {
		return nil, fmt.Errorf("node seclint: %v\n%s", err, buf.Bytes())
	}
